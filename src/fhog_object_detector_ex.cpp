...
int main(int argc, char **argv) {
  ...
  try {
    ...
    // where it is.
    if (argc != 2) {
      cout << "Give the path to the examples/faces directory as the argument "
              "to this"
           << endl;
      cout << "program.  For example, if you are in the examples folder then "
              "execute "
           << endl;
    ...
    scanner.set_detection_window_size(400, 50);
    structural_object_detection_trainer<image_scanner_type> trainer(scanner);

    // Set this to the number of processing cores on your machine.
    trainer.set_num_threads(4);

    // The trainer is a kind of support vector machine and therefore has the
    // usual SVM
    // C parameter.  In general, a bigger C encourages it to fit the training
    // data
    ...
    // of
    // images you haven't trained on.  Don't just leave the value set at 1.  Try
    // a few
    ...
    // The trainer will run until the "risk gap" is less than 0.01.  Smaller
    // values
    // make the trainer solve the SVM optimization problem more accurately but
    // will
    // take longer to train.  For most problems a value in the range of 0.1 to
    // 0.01 is
    // plenty accurate.  Also, when in verbose mode the risk gap is printed on
    // each
    ...
    // Now we run the trainer.  For this example, it should take on the order of
    // 10
    // seconds to train.
    object_detector<image_scanner_type> detector =
        trainer.train(images_train, face_boxes_train);

    // Now that we have a face detector we can test it.  The first statement
    // tests it
    // on the training data.  It will print the precision, recall, and then
    // average precision.
    cout << "training results: "
         << test_object_detection_function(detector, images_train,
                                           face_boxes_train)
         << endl;
    // However, to get an idea if it really worked without overfitting we need
    // to run
    // it on images it wasn't trained on.  The next line does this.  Happily, we
    // see
    // that the object detector works perfectly on the testing images.
    cout << "testing results:  " << test_object_detection_function(
                                        detector, images_test, face_boxes_test)
         << endl;

    // Like everything in dlib, you can save your detector to disk using the
    // serialize() function.
    serialize("fakegun.svm") << detector;

    // Then you can recall it using the deserialize() function.
    object_detector<image_scanner_type> detector2;
    deserialize("fakegun.svm") >> detector2;

    // If you have read any papers that use HOG you have probably seen the nice
    // looking
    // "sticks" visualization of a learned HOG detector.  This next line creates
    // a
    // window with such a visualization of our detector.  It should look
    // somewhat like
    // a face.
    image_window hogwin(draw_fhog(detector2), "Learned fHOG detector");
    cin.get();

    // Now for the really fun part.  Let's display the testing images on the
    // screen and
    // show the output of the face detector overlaid on each image.  You will
    // see that
    ...
    */
    ...
    // Now let's talk about some optional features of this training tool as well
    // as some
    // important points you should understand.
    //
    // The first thing that should be pointed out is that, since this is a
    // sliding
    // window classifier, it can't output an arbitrary rectangle as a detection.
    // In
    // this example our sliding window is 80 by 80 pixels and is run over an
    // image
    // pyramid.  This means that it can only output detections that are at least
    // 80 by
    // 80 pixels in size (recall that this is why we upsampled the images after
    // loading
    // them).  It also means that the aspect ratio of the outputs is 1.  So if,
    // for example, you had a box in your training data that was 200 pixels by
    // 10
    // pixels then it would simply be impossible for the detector to learn to
    // detect
    // it.  Similarly, if you had a really small box it would be unable to learn
    // to
    // detect it.
    //
    // So the training code performs an input validation check on the training
    // data and
    // will throw an exception if it detects any boxes that are impossible to
    // detect
    // given your setting of scanning window size and image pyramid resolution.
    // You
    // can use a statement like:
    //   remove_unobtainable_rectangles(trainer, images_train, face_boxes_train)
    // to automatically discard these impossible boxes from your training
    // dataset
    // before running the trainer.  This will avoid getting the "impossible box"
    // exception.  However, I would recommend you be careful that you are not
    // throwing
    // away truth boxes you really care about.  The
    // remove_unobtainable_rectangles()
    // will return the set of removed rectangles so you can visually inspect
    // them and
    ...
    // training data it is critical that you label all the objects you want to
    // detect.
    // So for example, if you are making a face detector then you must mark all
    // the
    // faces in each image.  However, sometimes there are objects in images you
    // are
    // unsure about or simply don't care if the detector identifies or not.  For
    // these
    // objects you can pass in a set of "ignore boxes" as a third argument to
    // the
    // trainer.train() function.  The trainer will simply disregard any
    // detections that
    // happen to hit these boxes.
    //
    // Another useful thing you can do is evaluate multiple HOG detectors
    // together. The
    // benefit of this is increased testing speed since it avoids recomputing
    // the HOG
    // features for each run of the detector.  You do this by storing your
    // detectors
    ...
    std::vector<rectangle> dets = evaluate_detectors(my_detectors,
    images_train[0]);
    */
    //
    //
    // Finally, you can add a nuclear norm regularizer to the SVM trainer.
    // Doing has
    // two benefits.  First, it can cause the learned HOG detector to be
    // composed of
    // separable filters and therefore makes it execute faster when detecting
    // objects.
    // It can also help with generalization since it tends to make the learned
    // HOG
    // filters smoother.  To enable this option you call the following function
    // before
    // you create the trainer object:
    //    scanner.set_nuclear_norm_regularization_strength(1.0);
    // The argument determines how important it is to have a small nuclear norm.
    // A
    // bigger regularization strength means it is more important.  The smaller
    // the
    // nuclear norm the smoother and faster the learned HOG filters will be, but
    // if the
    // regularization strength value is too large then the SVM will not fit the
    // data
    ...
    // You can also control how many filters there are by explicitly
    // thresholding the
    // singular values of the filters like this:
    // detector = threshold_filter_singular_values(detector,0.1);

    // That removes filter components with singular values less than 0.1.  The
    // bigger
